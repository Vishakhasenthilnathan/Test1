...
namespace ThoughtWorks
{
    class HitBatsman : IBatsman
    {
        ...
        int[] PossibleRuns;
        public HitBatsman(int NumberOfOvers, int ToBeChased)
        {
            _NumberofOvers = NumberOfOvers;
            _ToBeChased = ToBeChased;
            Bowler bowler = new Bowler();
            Random random = new Random();
            PossibleRuns = new int[] { 0, 4,6};
        }
        //char temp = passwordString[random.Next(0, passwordString.Length)];
